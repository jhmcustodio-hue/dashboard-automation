...
def test_azure_blob_publisher_returns_full_url_when_base_url_configured():
    container_client = MagicMock()
    publisher = AzureBlobPublisher(
        container_client=container_client, base_url="https://dashboards.empresa.com"
    )
    publicacao = PublicacaoConfig(destino="azure-blob", slug="comissoes-mensal")

    url = publisher.publish_latest("<html></html>", publicacao)

    assert url == "https://dashboards.empresa.com/comissoes-mensal/index.html"
    _, kwargs = container_client.upload_blob.call_args
    assert kwargs["name"] == "comissoes-mensal/index.html"
...
def test_databricks_native_publisher_returns_full_url_when_workspace_host_configured():
    workspace_client = MagicMock()
    publisher = DatabricksNativePublisher(
        workspace_client=workspace_client,
        base_path="/Workspace/dashboards",
    ...
    assert url == (
        "https://adb-123.azuredatabricks.net/Workspace/dashboards/comissoes-mensal/index.html"
    )
    _, kwargs = workspace_client.workspace.upload.call_args
    assert kwargs["path"] == "/Workspace/dashboards/comissoes-mensal/index.html"
